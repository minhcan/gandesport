           ...
           $category['name'] = str_replace("'", "\\'", $category['name']);
 				...
 				case 'featured-category-item':
 					...
 					$input .= '<div class="table-responsive">
							...
							$img = $placeholder;
							if(isset($categories_featured['image'])){
								$img = $this->model_tool_image->resize($categories_featured['image'], 60, 60);
								$var_img = $categories_featured['image'];
							}

							$input .= '<tr>';

							$input .= '<td class="text-left"><a href="" id="thumb-image" data-toggle="image" class="img-thumbnail">';
							$input .= '<img src="'.$img.'" alt="" title="" data-placeholder="'.$placeholder.'" /></a>';

							$input .= '     <input type="hidden" name="categories_featured[image]" value="'.$var_img.'" id="input-image" /></td>';

		                	$input .= '<td class="text-left">';
							$input .= '		<select class="form-control" name="categories_featured[id]">';
											    foreach ($categories as $category) {
											    	if(isset($categories_featured['id'])){
											    		$selected = ($categories_featured['id'] == $category['category_id'])?"selected='selected'":'';
											    	} else {
											    		$selected = '';
											    	}
